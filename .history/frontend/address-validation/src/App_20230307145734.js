    ...
    <div className=" items-center justify-between min-h-screen  ">
      <div className="  py-10 flex flex-col items-center border-2 border-black">
        ...
        <div className=" bg-green-300 mt-3">
          <select className=" p-5" multiple value={countryList} onChange={handleSelectedCountry}>
            {countries.map((country) => (
              <option className=" cursor-pointer uppercase tracking-widest p-2" key={country.name} value={country.name}>
                {country.name}
              </option>
            ))}
          </select>
